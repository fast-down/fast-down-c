...
#[unsafe(no_mangle)]
pub extern "C" fn cancellation_token_retain(
...
#[unsafe(no_mangle)]
pub extern "C" fn cancellation_token_release(ptr: *mut *mut CancellationToken) {
    if ptr.is_null() {
        return;
    }
    unsafe {
...
/// 创建子取消令牌
#[unsafe(no_mangle)]
pub extern "C" fn cancellation_token_child(
    ptr: *const CancellationToken,
) -> *mut CancellationToken {
    if ptr.is_null() {
        return std::ptr::null_mut();
    ...
    Box::into_raw(Box::new(CancellationToken {
        token: token.token.child_token(),
    }))
}

/// 触发取消
#[unsafe(no_mangle)]
pub extern "C" fn cancellation_token_cancel(ptr: *const CancellationToken) {
...
/// 查询是否已取消
#[unsafe(no_mangle)]
pub extern "C" fn cancellation_token_is_cancelled(ptr: *const CancellationToken) -> bool {
    unsafe {
        ptr.as_ref()
